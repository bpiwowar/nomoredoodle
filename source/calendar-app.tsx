...
import { CalendarSlot, CalendarEvent, calculateSlotStatus, doRangesIntersect } from './events'
...
function TimeSlotManager({slots: _slots, events, fillForm}: {slots: CalendarSlot[], events: CalendarEvent[], fillForm: (slots: CalendarSlot[]) => Promise<void>}) {
	...
	const computedSlots = useMemo(() => {
		return slots.map(slot => {
			if (slot.overridden) return slot;
	...
	const getIntersectingEvents = (slot: CalendarSlot) => {
		return events.filter(event => doRangesIntersect(slot, event));
	};
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
								...
								<div key={slot.id} className={`border rounded p-4 ${statusColors[slot.status]}`}>
									<div className="flex justify-between items-start">
										...
										<div className="flex space-x-2">
											<select
												value={slot.status}
												onChange={(e) => handleSlotStatusChange(slot.id, e.target.value as CalendarSlot["status"])}
				...
				<div className="content-center">
					<button type="button" onClick={() => {
						...
						fillForm(computedSlots).then(() => {}).catch((e) => {
							console.error("Error when filling the form", e)
						})
...
export function createApp(slots: CalendarSlot[], events: CalendarEvent[], fillForm: (slots: CalendarSlot[]) => Promise<void>) {
	...
	if (!wrapper) {
		...
		shadowRoot.appendChild(otherstyle);
	}
